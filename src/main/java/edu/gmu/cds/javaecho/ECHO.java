...
import edu.gmu.cds.javaecho.predicate.List;
import edu.gmu.cds.javaecho.predicate.Predicate;
import java.io.*;
...
final class ECHO {
  ...
  public final static int defaulMaxFlips = 100; // RCT added
  public final static int defaulMaxTries = 100; // RCT added
  ...
  //
  private Map units_ = new HashMap();

  //
  // the list of linked ECHOunits is stored in links_
  //
  private Map links_ = new HashMap();
  ...
  //
  private ArrayList competitors_ = new ArrayList();
  ...
  private void addExplanation(List propList, String explainee, float weight)
  {
    ...
    //
    ArrayList temp = new ArrayList(pSize);
  ...
  private void calculateCompetitors()
  {
    ...
    Iterator allUnits = units_.values().iterator();

    while (allUnits.hasNext()) {
      ECHOunit u = (ECHOunit)allUnits.next();
      //msg("checking to see who competes to explain "+u.name());
      ArrayList pairs = allDifferentPairs(u.allExplainedByUnits());
      //msg("all pairs = "+pairs);
      //
      // now prune pairs
      //
      ...
      Iterator e = pairs.iterator();
      while (e.hasNext()) {
	UnitPair up = (UnitPair)e.next();
	ECHOunit p = up.first();
  ...
  //
  // PRE: pairs is a ArrayList of UnitPairs holding all pairs of the explainers
  //      of E
  // POST: removes all pairs from the pair ArrayList that do *not* compete to 
  //       explain E
  //
  private void competitionPrunePairs(ArrayList pairs, ECHOunit E)
  {
    Assert.notNull(pairs);
    Assert.notNull(E);
    for(int i=0; i<pairs.size(); ++i) {
      //msg("i="+i);
      UnitPair up = (UnitPair)pairs.get(i); // .elementAt(i);
      if (up.same() || !competingPair(up,E)) {
	pairs.remove(i); //.removeElementAt(i);
	--i;
      } // if
    } // for
  }
  ...
  {
    boolean PexplainsQ = p.allExplainedByUnits().containsKey(q); // .contains(q);
    boolean QexplainsP = q.allExplainedByUnits().containsKey(p); // .contains(p);
    ...
    competitors_.add(new UnitPair(p,q));
  ...
  // POST: returns a ArrayList of all pairs (stored as UnitPairs) of the 
  //       elements in h
  //
  private ArrayList allDifferentPairs(Map h)
  {
    ArrayList result = new ArrayList(h.size());
    ArrayList temp = new ArrayList(h.size());
    Iterator a = h.values().iterator();
  
    while (a.hasNext()) {
      ECHOunit u1 = (ECHOunit)a.next();
      temp.add(u1);
      Iterator b = h.values().iterator();
      while (b.hasNext()) {
	ECHOunit u2 = (ECHOunit)b.next();
	if (!temp.contains(u2)) {
...
/*
  ...
  //       given weight on the links
  //
  private void makeAllLinks(ArrayList propPairs)
  {
...
*/
  ...
  //
  private ArrayList explainedByBoth(ECHOunit a, ECHOunit b)
  {
  ...
  //
  private int countMatches(Iterator e, Map g)    
  {
    int count = 0;
    while (e.hasNext()) {
      if (g.containsKey(e.next()))
	++count;
    } // while
    return count;
  }

  private ArrayList hashUnion(Map h, Map g)
  {
    ArrayList dup = duplicates(h,g);
    ArrayList result = new ArrayList();
    Iterator eh = h.entrySet().iterator();
    while (eh.hasNext()) {
      result.add(eh.next());
    } // while

    Iterator eg = g.entrySet().iterator();
    while (eg.hasNext()) {
      Object x = eg.next();
      if (!dup.contains(x))
	result.add(x);
  ...
  private ArrayList duplicates(Map h, Map g)
  {
    ArrayList result = new ArrayList();
    Iterator e = h.entrySet().iterator();
    while (e.hasNext()) {
      Map.Entry x = (Map.Entry)e.next();
      if (g.containsKey(x.getKey()))
	result.add(x);
    } // while
    return result;
  }
...
//////////////////////////////////////////////////////////////////
  ...
  {
    String result = "";
    Iterator e = units_.keySet().iterator(); 
  ...
  {
    String result = "";
    Iterator e = links_.keySet().iterator();
    for(int i=0; e.hasNext(); ++i) {
      Link l = (Link)links_.get(e.next());
      result += "link("+i+"): "+l+"\n";
    } // while
    return result;
  }
  ...
  {
    Iterator e = competitors_.iterator();
    if (!e.hasNext())
...
//
  ...
  public void checkInvariant()
  {
    ...
    if (invariantCheckingOn_) {
      ...
      Iterator ue = units_.keySet().iterator(); 
      while (ue.hasNext()) {
	Object u = ue.next();
	Assert.isTrue(u instanceof ECHOunit,
      ...
      Iterator le = links_.keySet().iterator();
      while (le.hasNext()) {
	Object l = le.next();

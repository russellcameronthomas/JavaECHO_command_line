...
final class GreedySolver extends ECHOsolver {
  ...
  private final int maxFlips_;
  private final int maxTries_;

  private final java.util.Random rand_ = new java.util.Random();
  ...
  GreedySolver(ECHO echo, int maxFlips, int maxTries)
  {
    ...
    getLinks();
    getUnitLinks();
    accepted_ = new ArrayList(units_.length);
    rejected_ = new ArrayList(units_.length);
  }
  ...
  {
    Iterator e = echo_.allUnits().keySet().iterator();
  ...
  private void getLinks()
  {
    ...
    Iterator e = echo_.allLinks().keySet().iterator();
  ...
  private void tally(boolean[] partition)
  {
    ...
    for(int i=0; i<partition.length; ++i) {
      if (partition[i])
	accepted_.add(units_[i]);
      else
	rejected_.add(units_[i]);
    } // for
    if (rejected_.contains(echo_.getSpecialUnit())) {
      Object temp = rejected_;
      rejected_ = accepted_;
      accepted_ = (ArrayList)temp;
    } // if
  }
